...
def train_single_batch(model: nn.Module, features: torch.Tensor, lengths: torch.Tensor,
    ...
    predictions, targets, lengths = model(features, lengths)
...
def train_single_epoch(model, epoch, step, train_loader, optimizer, scheduler, criterion, device, config):
    ...
    for batch_index, data in enumerate(train_loader):
        ...
        loss = train_single_batch(model=model, features=features, lengths=lengths,
                                  optimizer=optimizer, criterion=criterion, device=device,
                                  batch_index=batch_index, config=config,
                                  scheduler=scheduler)
        loss = loss * config["hparams"]["loss"]["accumulation_steps"]  # When using gradient accumulation loss is scaled
        running_loss += loss
...
def train(model: APC, optimizer: optim.Optimizer, criterion: Callable, train_loader: DataLoader,
    ...
    for epoch in tqdm(range(epochs), unit="Epoch", position=0, leave=True):
        step, log_dict = train_single_epoch(model, epoch, step, train_loader, optimizer, scheduler, criterion, device,
                                            config)
