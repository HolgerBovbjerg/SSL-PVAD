...
def load_speaker_embedding(item, metadata: pd.DataFrame, speaker_embeddings: dict):
    ...
    return waveform, labels_filestream, speaker_embedding, sampling_rate, target_speaker_id, audio_path


def compute_similarity(item, speaker_encoder):
    waveform, labels_filestream, speaker_embedding, sampling_rate, target_speaker_id, audio_path = item
    rate = 2.5
    min_coverage = 0.5
    ...
    return waveform, sampling_rate, labels_filestream, similarity, target_speaker_id, audio_path


def extract_features(item, feature_extractor):
    waveform, sampling_rate, labels_filestream, similarity, target_speaker_id, audio_path = item
    ...
    return features, sampling_rate, labels_filestream, similarity, target_speaker_id, audio_path


def load_labels(item):
    features, sampling_rate, labels_filestream, similarity, target_speaker_id, audio_path = item
    ...
    return features, labels, similarity, target_speaker_id, sampling_rate, audio_path



def segment_features(item, segment_size, drop_last: bool = False,
                     min_length: int = 0):
    features, labels, similarity, target_speaker_id, sampling_rate, audio_path = item
    features = features.split(segment_size, dim=-1)
    labels = labels.split(segment_size, dim=-1)
    similarity = similarity.split(segment_size, dim=-1)
    if (drop_last or (len(features[-1]) < min_length) and (len(features) > 1)):
        features = features[:-1]
        labels = labels[:-1]
        similarity = similarity[:-1]

    output = [(feature_segment, label_segment, similarity_segment, target_speaker_id)
              for feature_segment, label_segment, similarity_segment in zip(features, labels, similarity)]
...
def pad_collate(batch):
    ...
    similarity = [data[2] for data in batch[0]]
...
def build_libriconcat_datapipe(data_sets, feature_extractor, waveforms_dir: str = "Waveforms/",
                               ...
                               embeddings_filename: str = "speaker_embeddings.pt",
    ...
    if load_from == "features":
        ...
        datapipe = datapipe.map(partial(compute_similarity, speaker_encoder=speaker_encoder))
