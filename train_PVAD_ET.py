...
def training_pipeline(config):
    ...
    metric_scores = {"test_val_" + k + "_last": val for k, val in metric_scores.items()}
    ...
    metric_scores = {"test_val_" + k + "_best": val for k, val in metric_scores.items()}
    log_dict.update(metric_scores)
    log(log_dict, final_step, config)
...
if __name__ == "__main__":
    parser = ArgumentParser("Training and evaluation script for PVAD 1.0 ET model.")
