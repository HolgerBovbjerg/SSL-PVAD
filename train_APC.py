...
def training_pipeline(config):
    ...
    if config["hparams"]["scheduler"]["scheduler_type"] is not None:
        if config["hparams"]["scheduler"]["steps_per_epoch"]:
            steps_per_epoch = config["hparams"]["scheduler"]["steps_per_epoch"]
            total_iters = steps_per_epoch * max(1, (config["hparams"]["n_epochs"]))
            scheduler = get_scheduler(optimizer,
                                      scheduler_type=config["hparams"]["scheduler"]["scheduler_type"],
                                      t_max=total_iters,
                                      **config["hparams"]["scheduler"]["scheduler_kwargs"])
        else:
            steps_per_epoch = ceil(len(train_loader) / config["hparams"]["loss"]["accumulation_steps"])
            total_iters = steps_per_epoch * max(1, (config["hparams"]["n_epochs"]))
